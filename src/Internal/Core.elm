...
-}
type Log
    = SetTimer RequestId Float
    | ResolveTimer RequestId
...
{-| Special Cmds generated by TEPA implicitly.
-}
type AppCmd
    = PushPath
        { key : NavKey
                    ...
                    List.map
                        (\f m ->
                            case f m of
                                Nothing -> Nothing
                                Just (rid, sub1) ->
                                    Just (rid, Sub.map (mapMsg o.wrap) sub1)
                        )
                        newContext1.subs
                }
            , cmds = eff1.cmds
...
listen :
    ...
    , state : e -> List (Promise c m e Void)
    }
    -> Promise c m e Void
listen { name, subscription, state } =
    Promise <|
        \context ->
            let
                myRequestId =
